...
void UGameWaterSubsystem::Recompute(const UGridSubsystem& GridSubsystem)
{
	AWSIM_PERF_SCOPE(WaterRecompute);
	Capacity = 0.f;
	Consumption = 0.f;
	MaintenanceCost = 0.f;
...
void UGameWaterSubsystem::SettleDay()
{
...
UGamePlayerFundsSubsystem* UGameWaterSubsystem::ResolveFunds() const
{
	if (Funds)
	{
		return Funds;
...
UGridSubsystem* UGameWaterSubsystem::ResolveGrid() const
{
	if (Grid)
	{
		return Grid;
